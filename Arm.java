...
public class Arm
{
    ...
    public void draw()
    {
        ...
        out_str=(String)("vm1= " + (long)pwm1_val_1);
        ...
        out_str=(String)("vm2= " + (long)pwm1_val_2);
    ...
    public void directKinematic(){
        ...
        double  xa = (xj1 + xj2) / 2;
        double  ya = (yj1 + yj2) / 2;
        double la = Math.sqrt(Math.pow((xa - xj1),2) + Math.pow((ya - yj1),2));
        // distance between joints
        double d = Math.sqrt( Math.pow( (xj2 - xj1),2) + Math.pow((yj2 - yj1),2));
        if (d<2*r){
            valid_state = true;
            // half distance between tool positions
            double  h =  Math.sqrt(Math.pow((r),2) - la);
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        // elbows positions
        double alpha1 = Math.PI/2 - Math.PI + Math.atan2(yt-ym1,xt-xm1);
        xj1 = (xt+xm1)/2 + h1 * Math.cos(alpha1);
        yj1 = (yt+ym1)/2 + h1 * Math.sin(alpha1);
        ...
        // elbows positions
        double alpha2 = Math.PI/2 - Math.PI + Math.atan2(yt-ym2,xt-xm2);
        xj2 = (xt+xm2)/2 - h2 * Math.cos(alpha2);
        yj2 = (yt+ym2)/2 - h2 * Math.sin(alpha2);
